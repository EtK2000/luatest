...
---@param moduleName string
---@return string
local function getModuleRemoteUrl(installerConfig, moduleName)
	return installerConfig.repoPrefix .. 'utils/' .. moduleName .. '_utils.lua'
end

---Download `http_utils` and load it
---@param installerConfig InstallerConfig
---@param utilsDir string
local function downloadAndInjectHttpUtils(installerConfig, utilsDir)
	local httpUtilsPath = getModuleLocalPath(utilsDir, 'http')

	-- Download if needed or requested
	if installerConfig.redownload or not fs.exists(httpUtilsPath) then
		local httpUtilsUrl = getModuleRemoteUrl(installerConfig, 'http')
...
---@param installerConfig InstallerConfig
---@param utilsDir string
local function downloadAndInjectOsUtils(installerConfig, utilsDir)
	local osUtilsPath = getModuleLocalPath(utilsDir, 'os')

	-- Download if needed or requested
	if installerConfig.redownload or not fs.exists(osUtilsPath) then
		local ok, errorMessage = pcall(
			http.download,
			getModuleRemoteUrl(installerConfig, 'os'),
			osUtilsPath
		)
		if not ok then
...
---@param installerConfig InstallerConfig
---@param utilsDir string
local function downloadAndInjectPermissionUtils(installerConfig, utilsDir)
	local permissionUtilsPath = getModuleLocalPath(utilsDir, 'permission')

	if installerConfig.redownload or not fs.exists(permissionUtilsPath) then
		local ok, errorMessage = os.try(
			http.download,
			getModuleRemoteUrl(installerConfig, 'permission'),
			permissionUtilsPath
		)
		if not ok then
...
---@param utilsDir string
---@return fun()
local function downloadAndInjectRemainingUtils(installerConfig, utilsDir)
	return function()
		...
		for _, moduleName in ipairs(UTILS_FILES_TO_DOWNLOAD_NORMALLY) do
			...
			if installerConfig.redownload or not fs.exists(modulePath) then
				local ok, errorMessage = os.try(
					http.download,
					getModuleRemoteUrl(installerConfig, moduleName),
		...
		downloadAndInjectPermissionUtils(installerConfig, utilsDir)
	end
end
...
return function(installerConfig, utilsDir)
	...
	downloadAndInjectHttpUtils(installerConfig, utilsDir)

	-- Now download os_utils.lua for `os.try`
	downloadAndInjectOsUtils(installerConfig, utilsDir)

	-- Download everything else with a nicer loading
	os.clear()
	os.catchTerminate()
	parallel.waitForAny(
		downloadAndInjectRemainingUtils(installerConfig, utilsDir),
		letUserCancel,
		os.loading
	)
