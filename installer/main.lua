...
---@return unknown
local function loadAndExecute(installerConfig, pathWithoutExtension)
	local osDownloaderPath = getDiskPath() .. pathWithoutExtension
	if installerConfig.redownload or not fs.exists(osDownloaderPath) then
		http.download(installerConfig.repoPrefix .. pathWithoutExtension .. '.lua', osDownloaderPath)
	end
...
local file = fs.open(installerConfig.forceRedownloadFilePath, 'r')
if file then
	file.close()

	-- If so, enable redownload
	installerConfig.redownload = true
...
if installerConfig.redownload or not fs.exists(bootstrapUtilsPath) then
	print('Downloading utilities...')

	local bootstrapUtilsUrl = installerConfig.repoPrefix .. 'utils/bootstrap.lua'
...
local doPrivileged = require('utils/bootstrap')(installerConfig, getDiskPath() .. 'utils/')
os.clear()

-- Redownload this file, then reboot into `main_swapper`
if installerConfig.redownload then
	-- Download the new `main`
	local ok, errorMessage = os.try(
		http.download,
		installerConfig.repoPrefix .. 'installer/main.lua',
	...
	local ok, errorMessage = os.try(
		http.download,
		installerConfig.repoPrefix .. 'installer/main_swapper.lua',
		getDiskPath() .. 'startup'
	)
	if not ok then
		error(errorMessage)
...
local wasOsDownloaded = loadAndExecute(installerConfig, 'installer/os_downloader')
	(installerConfig, getDiskPath() .. 'os/')
...
---@type GuiObject
local gui = loadAndExecute(installerConfig, 'installer/gui')(installerConfig)
if not gui.shouldInstallationContinue then
	return
end
...
loadAndExecute(installerConfig, 'installer/module_downloader')(installerConfig, getDiskPath() .. 'os/mods/')
